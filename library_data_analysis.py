...
# 读取Excel数据
df = pd.read_excel("E:\\trae\\3.20\\main\\图书馆读者借阅消费数据.xlsx")
...
# 注：原数据中没有私教/专属服务类项目，这里模拟添加一些专属服务数据进行分析演示
# 在实际场景中，这些数据应该已经存在于原始数据中

print("说明：原始数据中无'私教/专属服务'项目，以下分析基于模拟的专属服务数据进行演示")
print("      在实际业务中，专属服务可能包括：阅读指导、研究咨询、个性化推荐等")

# 模拟专属服务数据（基于专题讲座和文创购买，加上一些自定义服务）
np.random.seed(42)
...
for idx, row in top3_librarians.iterrows():
    print(f"   第{list(top3_librarians.index).index(idx)+1}名：{row['馆员ID']} - 消费额：{row['消费总额(元)']:.2f}元，贡献度：{row['贡献度(%)']}%")
...
# 创建Excel写入器
output_path = "E:\\trae\\3.20\\kimi\\图书馆数据分析结果.xlsx"
